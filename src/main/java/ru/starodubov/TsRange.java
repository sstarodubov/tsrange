...
public final class TsRange implements Comparable<TsRange> {
    ...
    public static TsRange parseRange(final String range) {
        ...
        final boolean lowerInc = switch (range.charAt(0)) {
            case '(' -> false;
            case '[' -> true;
            default -> throw new IllegalStateException();
        };

        final boolean upperInc = switch (range.charAt(range.length() - 1)) {
            case ')' -> false;
            case ']' -> true;
            default -> throw new IllegalStateException();
        };
        ...
        final LocalDateTime lower = parseTimestamp(range, 1, commaIdx - 1);
        final LocalDateTime upper = parseTimestamp(range, commaIdx + 1, range.length() - commaIdx - 2);
